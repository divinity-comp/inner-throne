...
function rcp_is_post_taxonomy_restricted( $post_id, $taxonomy, $user_id = null ) {
	...
	foreach( $terms as $term ) {
		...
		if( $subscriptions && count( array_intersect( rcp_get_customer_membership_level_ids( $customer->get_id() ), $subscriptions ) ) ) {
			$restricted = false;
			break;
		}
		...
		if( $access_level > 0 && $customer->has_access_level( $access_level ) ) {
			$restricted = false;
			break;
		}
	}
