...
core bbPress notification templates, and is fully configurable in its settings screen and extensible via hooks.
...
messages to other roles, which could then be characterized as spam. To allow your users to opt-out from receiving notifications, please consider purchasing
the [Opt-out Add-on](http://usestrict.net/product/bbpress-notify-no-spam-opt-out-add-on/) or disable per-role notifications in favor of Overriding Core Subscriptions.
