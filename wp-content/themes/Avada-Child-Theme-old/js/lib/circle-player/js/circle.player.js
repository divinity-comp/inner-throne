...
var CirclePlayer = function(jPlayerSelector, media, options) {
	...
	this.player = $j(jPlayerSelector);
	this.media = $j.extend({}, media);
	this.options = $j.extend(true, {}, defaults, options); // Deep copy
	...
	this.jq = {};
	$j.each(cssSelector, function(entity, cssSel) {
		self.jq[entity] = $j(self.options.cssSelectorAncestor + " " + cssSel);
	});
...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind($j.jPlayer.event.ready + this.eventNamespace, function(event) {
			if(event.jPlayer.html.used && event.jPlayer.html.audio.available) {
				self.audio = $j(this).data("jPlayer").htmlElement.audio;
			}
			$j(this).jPlayer("setMedia", self.media);
			self._initCircleControl();
		});

		this.player.bind($j.jPlayer.event.play + this.eventNamespace, function(event) {
			$j(this).jPlayer("pauseOthers");
		});

		// This event fired as play time increments
		this.player.bind($j.jPlayer.event.timeupdate + this.eventNamespace, function(event) {
			if (!self.dragging) {
		...
		this.player.bind($j.jPlayer.event.progress + this.eventNamespace, function(event) {
			var percent = 0;
		...
		this.player.bind($j.jPlayer.event.ended + this.eventNamespace, function(event) {
			self._resetSolution();
		});
	},
