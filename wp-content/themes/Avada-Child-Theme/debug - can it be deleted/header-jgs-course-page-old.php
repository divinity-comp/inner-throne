...
<html class="<?php echo ( Avada()->settings->get( 'smooth_scrolling' ) ) ? 'no-overflow-y' : ''; ?>" <?php language_attributes(); ?>>
	...
	<?php wp_enqueue_script("jquery-cookie", get_stylesheet_directory_uri().'/js/jquery-cookie/jquery.cookie.js', array(), '0'); ?>
	...
	<script type="text/javascript">
		...
		jQuery('document').ready(function($j){
			...
			$j('body.home .slides li:first-child').hide();
				...
				if(field_value !== "" && field_value !== echo_system_msg) { 
					if(field_value.length > minimum_length) {
						...
						if(field_value.length < (minimum_length/2)) {
							jgs_status_popup("<p>Honestly, you're trying to get off the hook easy, aren't you? Put some more soul into it. You must write more.","show",0,false);
						}
						else if (field_value.length < (minimum_length*0.75)){
							jgs_status_popup("<p>You're getting close. But these are the Great Gates, and you must make a real effort!","show",0,false);
						}
						else {
							jgs_status_popup("<p>Great! Just a few more words, and I will let you pass! I must know your heart properly.","show",0,false);
						}
					}
				}
				else {
					jgs_status_popup("<p>You haven't even written anything! Away with you, wretch! You are NOT worthy.","show",0,false);
				}
			});
			...
			$j("#wolf_button").on("click", function(e) {
				if($j(this).children('a').attr('href') === "#") { //Link to new section on same page
					//console.log("default");
					e.preventDefault();
					step = progress_step;
					week_id = curr_week;
			...
			//Retrieve COACHING-system
			$j('#jgs_mystery_box_menuitem').on('click', function(e) {
				if($j('#jgs_mystery_box').length > 0) {
					if($j('#jgs_mystery_box').is(':visible')) {
						console.log('box is visible');
						$j('#jgs_mystery_box').fadeOut(500);
					}
					else {
						console.log('box is invisible');
						$j('#jgs_mystery_box').fadeIn(500);
					}
				}
				else {
					e.preventDefault();
					...
					jgs_status_popup("<p>Opening Marion's Box...</p>",'show',0,false);
					...
					$j.ajax({
						...
						success: function(response) {
							if(response !== "") {         
								...
								$j('body').prepend('<div id="jgs_mystery_box" style="display: none;"></div>');
								$j('#jgs_mystery_box').html(response).fadeIn(500);
							}
						else {
			...
			$j('#acf_input_form :submit').click(function(event){
				...
				//console.log("form complete: " + form_complete);
				//console.log("require all: " + require);
			...
			$j(document).on('click', 'a[data-callback="init_dragon"]', function(e) {
				e.preventDefault();
				$j('#choice-sequence-bg').css('visibility','visible');
				$j('#choice-sequence').css('visibility','visible');
				$j('#choice-sequence').removeClass('hidden');
			...
			$j(document).on('click', '#challenges .challenge', function(e) {
				e.preventDefault();
				if($j(this).hasClass('showall')) {
					$j('#challenges .challenge.showall').find('.toggle').text('Click for Info');
					$j(this).removeClass('showall');
					$j(this).find('.complete').text('Click for Info');
				}
				else {
					$j('#challenges .challenge').find('.toggle').text('Click to Collapse');
					$j('#challenges .challenge').removeClass('showall');
					$j(this).addClass('showall');
					$j(this).find('.complete').text('Click to collapse');
				}
			});
			...
			$j(document).on('click', '#challenges .meta a', function(e) {
				...
				jgs_status_popup('<div class="challenge_check"><h3>Are you sure you want to claim this reward?</h3><p>Make sure you have actually completed it before clicking yes.</p><ul><li class="yes">Yes</li><li class="no">No</li></ul></di>','show',0,true);
			});		
...
<body <?php body_class(); ?> post_id="<?php echo $post->ID; ?>" week_id="<?php echo $week_id; ?>" prev_slug="<?php echo $prev_slug; ?>" next_slug="<?php echo $next_slug; ?>" progress_week="<?php echo $progress_week; ?>" progress_step="<?php echo $progress_step; ?>" user_id="<?php echo $user_id; ?>">
	<?php
