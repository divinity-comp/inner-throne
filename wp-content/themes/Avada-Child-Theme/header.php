...
-->
	...
	<meta name="p:domain_verify" content="3a06a682cbb933068672120986bbf1c6"/> <!-- Pinterest -->
	<?php Avada()->head->the_viewport(); ?>
	<?php wp_enqueue_script("js-cookie", get_stylesheet_directory_uri().'/js/js-cookie/js-cookie.js', array(), true); ?>
	
	<?php
		$add_css_files = get_field('ryit_add_css_files');
		if(!empty($add_css_files)) {
			foreach ($add_css_files as $file) {
				echo '<link rel="stylesheet" href="' . get_stylesheet_directory_uri() . '/' . $file['css_file'] . '"/>';
			}
		}
	?>
...
<body <?php body_class(); ?> user_id="<?php echo $user_id; ?>" post_id="<?php echo $post_id; ?>">
	<?php
	...
	<div id="wrapper" class="<?php echo esc_attr( $wrapper_class ); ?>">
		...
		<?php
			if(is_fellowship_page() || is_single() || is_archive()) {
